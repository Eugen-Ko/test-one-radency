...
export const elementToDo = ({ name, createDate, category, content, expDate, statArch, id }) => {
  ...
  const icons = ['create', 'archive', 'delete'].map(el => {
    const icon = createEl({ tagName: 'svg', className: `elementSVG ${el}`, attributes: { id: id, viewBox: "0 0 24 24", preserveAspectRatio: "none" } });
